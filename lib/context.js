...
/**
 * Assertion helper that throws HTTP errors using the same http-errors instance
 * as ctx.throw(), ensuring instanceof checks against the exported HttpError work
 * correctly even when http-assert bundles a different http-errors version.
 *
 * API matches http-assert (https://github.com/jshttp/http-assert).
 */
function assert (value, status, msg, opts) {
  if (value) return
  if (opts != null) throw createError(status, msg, opts)
  if (msg != null) throw createError(status, msg)
...
assert.deepEqual = function (a, b, status, msg, opts) {
  assert(util.isDeepStrictEqual(a, b), status, msg, opts)
}
