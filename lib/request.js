...
module.exports = {
  ...
  get ip () {
    if (!this[IP]) {
      const addr = this.ips[0] || this.socket.remoteAddress || ''
      // Strip port from X-Forwarded-For entries (e.g., "1.2.3.4:38242" or "[::1]:8080")
      // See: https://github.com/koajs/koa/issues/827
      if (addr.startsWith('[')) {
        // IPv6 with port: [::1]:8080 -> ::1
        this[IP] = addr.includes(']:') ? addr.slice(1, addr.indexOf(']')) : addr
      } else if (addr.includes(':')) {
        // IPv4 with port: 1.2.3.4:8080 -> 1.2.3.4 (not IPv6 which has multiple colons)
        const colonIndex = addr.indexOf(':')
        this[IP] = addr.indexOf(':', colonIndex + 1) === -1
          ? addr.slice(0, colonIndex)
          : addr
      } else {
        this[IP] = addr
      }
