...
/**
 * Setup proper abort handling for fetch streams to prevent memory leaks
 * @param {Stream.Readable} readable - The Node.js readable stream
 * @param {ReadableStream|Response} original - The original fetch object
 * @param {http.ServerResponse} res - The response object
 */
function setupStreamAbortHandling (readable, original, res) {
  const onClose = () => {
    ...
    if (original instanceof Response) {
      if (original.body && original.body instanceof ReadableStream) {
        if (!original.body.locked && typeof original.body.cancel === 'function') {
          original.body.cancel('Client disconnected')
        }
      }
      return
    }

    if (original instanceof ReadableStream) {
      if (!original.locked && typeof original.cancel === 'function') {
        original.cancel('Client disconnected')
      }
    }
  }
...
function respond (ctx) {
  ...
  if (body instanceof ReadableStream) {
    ...
    setupStreamAbortHandling(readable, body, res)
