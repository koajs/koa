...
/*
  ...
  Mounting middleware.
  This is a superset of app.use with the intention
  of having the same signature of connect/express.
...
app.use =
app.mount = function (prefix, middleware) {
  var self = this;
  var app;
  ...
  if (middleware instanceof Application) {
    app = middleware;
    middleware = app.middleware;
  }

  // If no prefix and no app, just mount the middleware
  if (!prefix && !app) {
    this.middleware.push(middleware);
    return this;
  }

  // If no preifx, but app, we have to change the prototype.
  if (!prefix && app) {
    this.middleware.push(function* () {
      this.setPrototype(app);

      yield 'next';

      this.setPrototype(self);
    }, middleware, function* () {
      this.setPrototype(self);
  ...
  if (prefix && !app) {
    this.middleware.push(function(){
      ...
      // not a match
      if (0 != this.url.indexOf(prefix)) return;
  ...
  throw TypeError('Need a middleware or app to mount.');
}
