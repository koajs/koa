...
function *respond(next) {
  ...
  var stream = body && 'function' == typeof body.pipe;

  // handle stream's error anyway
  if (stream && !~body.listeners('error').indexOf(this.onerror))
  ...
  if (status.empty[code]) {
    ...
    if (stream && body.close) body.close();
    return res.end();
  }
