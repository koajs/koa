...
/**
 * Use the given middleware `fn(s)`.
 *
 * @param {GeneratorFunction|Array} fns
...
app.use = function(fns){
  ...
  for(var i=0; i<fn.length; i++){
    var fn = fns[i];
    assert('GeneratorFunction' == fn.constructor.name, 'app.use() requires a generator function or an array of generator functions');
    debug('use %s', fn.name || '-');
