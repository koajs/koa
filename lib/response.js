...
module.exports = {
  ...
  /**
   * Handle any stream errors on this.body=
   * instead of waiting until the respond middleware.
   * Assumes every stream ever set on this.body=
   * is relevant to response, meaning you shouldn't
   * be swapping bodies except for error handling.
   *
   * We also "destroy" or "close" the stream if the socket
   * closes before the stream is finished to avoid any
   * possible leaks (such as file descriptor leaks).
   *
   * @param {Stream} stream
   * @api private
   */
  ...
  _handleStream: function(stream){
    ...
    function destroy(err) {
      if (err) onerror(err);
      if ('function' == typeof stream.destroy) stream.destroy();
      if ('function' == typeof stream.close) stream.close();
      cleanup();
    }
