...
module.exports = {
  ...
  set status(code) {
    ...
    assert(status[code], 'invalid status code: ' + code);
  ...
  setStatus: function(code, msg){
    assert('number' == typeof code, 'status code must be a number');
    if (msg) {
      assert('string' == typeof msg, 'status message must be a string');
      this.res.statusCode = code;
      this.res.statusMessage = msg;
    } else {
      assert(status[code], 'message is required for custom status');
      this.status = code;
    }
  }
};
