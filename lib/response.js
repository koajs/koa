...
module.exports = {
  ...
  /**
   * Send a file from location `filename`.
   * Primary purpose is to avoid having end-users use streams.
   *
   * @param {String} filename
   * @api public
   */
  ...
  sendfile: function *(filename){
    ...
    if (!this.type) this.type = mime.lookup(filename);
    this.length = stats.size;
    this.lastModified = stats.mtime;
    if (!this.etag) this.etag = etag(stats);
    if (this.req.method !== 'HEAD') this.body = fs.createReadStream(filename);
  },
...
/**
 * fs.stat as a yieldable,
 * but also adds the appropriate status code
 * so we can delegate the try/catch upstream
...
function stat(filename) {
  return function (done) {
    fs.stat(filename, function (err, stats) {
      // add 404 status code to ENOENT
      if (err && 'ENOENT' == err.code) err.status = 404;
      done(err, stats);
    })
  }
}
...
/**
 * Default etag for a file based on its stats.
 *
 * @param {Object} stats
 * @return {String}
 * @api private
