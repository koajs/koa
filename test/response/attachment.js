...
describe('ctx.attachment([filename])', function(){
  ...
  describe('when given a no-ascii filename', function(){
    it('should set the encodeURI filename param', function(){
      ...
      const str = 'attachment; filename=\"include-no-ascii-char-???-ok.png\";' +
        ' filename*=UTF-8\'\'' +
        'include-no-ascii-char-%E4%B8%AD%E6%96%87%E5%90%8D-ok.png';
      ctx.response.header['content-disposition'].should.equal(str);
    });
    ...
    it('should work with http client', function(done){
      ...
      request(app.listen())
        ...
        .expect('content-disposition', 'attachment; ' +
          'filename="include-no-ascii-char-???-ok.json"; filename*=UTF-8\'\'' +
          'include-no-ascii-char-%E4%B8%AD%E6%96%87%E5%90%8D-ok.json')
        .expect({foo: 'bar'})
        .expect(200, done);
    });
  });
